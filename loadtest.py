...
class LoadTest(object):
    def __init__(self, conf):
        ...
        self.hosts = self.config["hosts"].split(",")
    ...
    def upload_file(self, dbmgr, fileno, host, port, username, passwd, source, target):
        ...
        try:
            ...
            if localpath!=None and remotepath!=None:
                ...
                dbmgr.add_entry(dbmgr.connection(), fileno, host, port, username,"UNKNOWN","failed",traceback.format_exc())
    ...
    def run_test(self):
        ...
        while True:
            for host in self.hosts:
                hostinfo = host.split(":")
                users = self.config[host]["users"].split(",")
                active = self.config[host]["active"]
                if active == "false":
                    continue
                print(users)
                for user in users:
                    ...
                    userinfo = self.config[host][user]
                    files = self.config[host]["files"]
