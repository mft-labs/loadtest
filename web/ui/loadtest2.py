...
class DbManager(object):
    def __init__(self, logger, log_suffix, dbname,add_timestamp=True):
        self.logger = logger
        ...
        if add_timestamp:
            ...
            self.con = sqlite3.connect(f"{dbname}_{log_suffix}.db",check_same_thread=False)
            self.initialize()
        else:
...
def run_manager(conf):
    ...
    app = LoadTest(log_suffix, logger, conf,sftptester,running_threads)
